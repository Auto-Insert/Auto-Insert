...
public class ScrewInThePlug : SequenceStep
{
    ...
    private async Task MainSequence()
    {
        ...
        // Deatach tool
        await _stepperMotorService.MoveAsync(StepperMotorService.Motor.Tool, StepperMotorService.Direction.Clockwise, 25);
        await _uartService.AddDelayToBufferAsync(1);
        await _stepperMotorService.MoveAsync(StepperMotorService.Motor.Tool, StepperMotorService.Direction.Clockwise, 1000);
        ...
        await _uartService.AddDelayToBufferAsync(2);
        await _stepperMotorService.MoveAsync(StepperMotorService.Motor.Tool, StepperMotorService.Direction.Clockwise, 400);
        await _stepperMotorService.MoveAsync(StepperMotorService.Motor.Tool, StepperMotorService.Direction.AntiClockwise, 401);
        await _uartService.AddDelayToBufferAsync(5);
        ...
        await _uartService.WaitForStringAsync(">>>>Stepper 3 moved 1000 steps<<<<");
        _linearActuatorService.SetPosition(0);
        await _uartService.WaitForStringAsync(">>>>Stepper 1 moved 222 steps<<<<");
        _linearActuatorService.SetPosition(100);
        await _uartService.WaitForStringAsync(">>>>Stepper 3 moved 401 steps<<<<");
        _linearActuatorService.SetPosition(0);
